// Copyright (c) AerafalGit 2025.
// Jade licenses this file to you under the MIT license.
// See the license here https://github.com/AerafalGit/Jade/blob/main/LICENSE.

using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;

namespace Jade.Ecs.Components;

/// <summary>
/// Provides a registry for managing component metadata in the ECS (Entity Component System).
/// </summary>
public static unsafe class ComponentRegistry
{
    private static readonly Dictionary<Type, ComponentMetadata> s_metadataByType;
    private static readonly Dictionary<int, ComponentMetadata> s_metadataById;
    private static readonly Lock s_lock;

    private static int s_nextId;

    /// <summary>
    /// Initializes the static fields of the <see cref="ComponentRegistry"/> class.
    /// </summary>
    static ComponentRegistry()
    {
        s_metadataByType = [];
        s_metadataById = [];
        s_lock = new Lock();
        s_nextId = -1;
    }

    /// <summary>
    /// Retrieves the metadata for a component type.
    /// </summary>
    /// <typeparam name="T">The type of the component.</typeparam>
    /// <returns>The metadata associated with the component type.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ComponentMetadata GetMetadata<T>()
    {
        var type = typeof(T);

        return s_metadataByType.GetValueOrDefault(type, RegisterSlow<T>(type));
    }

    /// <summary>
    /// Retrieves the metadata for a component by its ID.
    /// </summary>
    /// <param name="id">The ID of the component.</param>
    /// <returns>The metadata associated with the component ID.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ComponentMetadata GetMetadata(int id)
    {
        lock (s_lock)
            return s_metadataById[id];
    }

    /// <summary>
    /// Retrieves all registered component IDs.
    /// </summary>
    /// <returns>An enumerable collection of all component IDs.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static IEnumerable<int> GetAllComponentIds()
    {
        lock (s_lock)
        {
            return s_metadataById.Keys;
        }
    }

    /// <summary>
    /// Registers a component type and creates its metadata.
    /// </summary>
    /// <typeparam name="T">The type of the component.</typeparam>
    /// <param name="type">The type of the component.</param>
    /// <returns>The metadata created for the component type.</returns>
    [MethodImpl(MethodImplOptions.NoInlining)]
    private static ComponentMetadata RegisterSlow<T>(Type type)
    {
        lock (s_lock)
        {
            if (s_metadataByType.TryGetValue(type, out var metadata))
                return metadata;

            var id = Interlocked.Increment(ref s_nextId);

            var isBlittable = !RuntimeHelpers.IsReferenceOrContainsReferences<T>();

            var size = isBlittable
                ? sizeof(T)
                : IntPtr.Size;

            var alignment = isBlittable
                ? CalculateAlignment<T>()
                : size;

            Func<int, ComponentArray<T>> factory = isBlittable
                ? capacity => new ComponentArrayUnmanaged<T>(size, alignment, capacity)
                : capacity => new ComponentArrayManaged<T>(capacity);

            metadata = new ComponentMetadata(id, size, factory);

            s_metadataByType[type] = metadata;
            s_metadataById[id] = metadata;

            return metadata;
        }
    }

    /// <summary>
    /// Calculates the alignment for a component type.
    /// </summary>
    /// <typeparam name="T">The type of the component.</typeparam>
    /// <returns>The calculated alignment value.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static int CalculateAlignment<T>()
    {
        var naturalAlignment = sizeof(ComponentAlignment<T>) - sizeof(T);

        if (naturalAlignment <= 0)
        {
            var size = sizeof(T);

            return size switch
            {
                1 => 1,
                2 => 2,
                <= 4 => 4,
                <= 8 => 8,
                _ => 16
            };
        }

        if ((naturalAlignment & (naturalAlignment - 1)) is not 0)
        {
            var powerOf2 = 1;

            while (powerOf2 < naturalAlignment)
                powerOf2 <<= 1;

            return Math.Min(powerOf2, 16);
        }

        return Math.Min(naturalAlignment, 16);
    }

    /// <summary>
    /// Represents the alignment of a component type.
    /// </summary>
    /// <typeparam name="T">The type of the component.</typeparam>
    [StructLayout(LayoutKind.Sequential)]
    private readonly struct ComponentAlignment<T>
    {
        /// <summary>
        /// Padding for alignment purposes.
        /// </summary>
        public readonly byte Padding;

        /// <summary>
        /// The actual component.
        /// </summary>
        public readonly T Component;
    }
}
